...
public class QueueModelController {
	...
	private static Logger log = Logger.getLogger(QueueModelController.class);
	...
	private ManagerControllerThread manager;

	public ManagerControllerThread getManager() {
		return manager;
	}

	public void setManager(ManagerControllerThread manager) {
	...
	private void logStart(long time) {
		...
		log.debug(String.format(placeholder, time));
	}
	...
	public void start() {
		...
		try {
			...
			log.error("Daemon init failed: " + ex.getMessage());
		}
		...
		do {
			...
			try {
				...
				log.error("Thread execution error: " + ex.getMessage());
			}

			log.debug("Queue size: " + Integer.toString(queue.size()));
